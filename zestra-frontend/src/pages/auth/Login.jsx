...
const Login = () => {
  ...
  const handleGoogleLogin = () => {
    ...
    window.location.href = `${apiBaseUrl}/auth/google/login?role=customer`;
  };
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-10 py-3 bg-(--surface) border border-(--border) rounded-[14px] focus:outline-none focus:border-(--primary) focus:ring-4 focus:ring-[rgba(16,185,129,0.15)] transition-all text-(--text) placeholder-(--text-muted)"
                placeholder="••••••••"
              />
              <button
                type="button"
                onClick={() => setShowPassword(!showPassword)}
                className="absolute inset-y-0 right-0 pr-3 flex items-center text-(--text-muted) hover:text-(--text) transition-colors focus:outline-none"
              >
                {showPassword ? (
