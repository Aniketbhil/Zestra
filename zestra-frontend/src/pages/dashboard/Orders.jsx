...
import { ChefHat, CheckCircle2, Utensils, Receipt, Banknote, Loader2, Clock, Check } from 'lucide-react';
...
const Orders = () => {
  ...
  // SMART FILTERING BASED ON PAYMENT STATUS
  // Counter payments stay here until confirmed
  const unpaidOrders = orders.filter(o => o.status === 'received' && o.payment_status !== 'paid');
  
  // Online payments instantly jump here even if technically "received" in the DB
  const preparingOrders = orders.filter(o => o.status === 'preparing' || (o.status === 'received' && o.payment_status === 'paid'));
  ...
  const OrderCard = ({ order }) => {
    ...
    return (
      <div className={`bg-(--surface) p-5 rounded-3xl border border-(--border)/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_15px_40px_rgb(0,0,0,0.08)] hover:-translate-y-1 transition-all duration-300 flex flex-col group relative overflow-hidden ${isUpdating ? 'opacity-70 animate-pulse pointer-events-none scale-[0.98]' : ''}`}>
        ...
        {/* Card Header (ID, Payment Badge, Time & Total) */}
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {order.payment_status === 'paid' ? (
                <span className="bg-emerald-100 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-md text-[10px] font-black tracking-wider uppercase flex items-center gap-1 shadow-sm">
                  <Check className="w-3 h-3" /> Paid
                </span>
              ) : (
                <span className="bg-amber-100 text-amber-700 border border-amber-200 px-2 py-0.5 rounded-md text-[10px] font-black tracking-wider uppercase flex items-center gap-1 shadow-sm">
        ...
        {/* Dynamic Action Buttons based on Status and Payment */}
        <div className="mt-auto relative z-10">
          
          {/* Action: Unpaid at Counter -> Confirm Payment & Move to Preparing */}
          {order.status === 'received' && order.payment_status !== 'paid' && (
            ...
            >
              ...
              {isUpdating ? 'Processing...' : 'Confirm Payment & Prep'}
            </button>
          )}

          {/* Action: Online Paid (Sitting in Preparing Column) -> Start Preparing */}
          {order.status === 'received' && order.payment_status === 'paid' && (
            <button 
              onClick={() => handleStatusChange(order.id, 'preparing')}
              disabled={isUpdating}
            ...
            >
              ...
              {isUpdating ? 'Starting...' : 'Acknowledge & Prepare'}
            </button>
          )}
          
          {/* Action: Actively Preparing -> Mark as Ready */}
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] font-sans">
      ...
      <div className="flex-1 overflow-x-auto overflow-y-hidden scrollbar-thin scrollbar-thumb-(--border) scrollbar-track-transparent pb-4 px-1 mask-linear-fade">
        <div className="flex gap-6 min-w-max h-full">
          ...
          {/* Column 1: Unpaid (Counter Orders only) */}
          <div className="w-85 flex flex-col bg-(--surface-secondary)/40 rounded-4xl border border-(--border)/60 overflow-hidden h-full shadow-sm relative">
            <div className="p-5 border-b border-blue-500/10 bg-linear-to-br from-blue-500/10 to-transparent flex items-center justify-between shrink-0">
              ...
              <span className="bg-white border border-blue-100 text-blue-600 px-3 py-1 rounded-full text-sm font-black shadow-sm">{unpaidOrders.length}</span>
            </div>
            <div className="p-4 flex-1 overflow-y-auto space-y-4 scrollbar-none">
              {unpaidOrders.map(order => <OrderCard key={order.id} order={order} />)}
          ...
          {/* Column 2: Preparing (Includes fresh Online Payments) */}
          <div className="w-85 flex flex-col bg-(--surface-secondary)/40 rounded-4xl border border-(--border)/60 overflow-hidden h-full shadow-sm relative">
